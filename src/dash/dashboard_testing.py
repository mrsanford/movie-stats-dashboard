...
def create_layout(app: Dash):
    ...
    header = html.Div(style={"fontFamily": "Calibri, sans-serif"}, children=[
        ...
        html.P(
            ...
            "MoVIZ does the rest.",
            style={"textAlign": "center", "fontSize": "16px", "marginTop": "-10px", "color": "#555"})
    ])

    cat_label = html.Div([
        html.Label("Select criteria you would like to filter:"),
        dcc.Dropdown(id="char-selector", options=filter_characteristics, multi=True,
    ...
    filter_summary = html.Div(id="filter-summary", style={"marginTop": "10px", "fontSize": "15px", "color": "#333"})

    stores = html.Div([
        dcc.Store(id="user-filters", data={}),
    ...
    graph_display = html.Div([
        html.Hr(),
        html.Div(id="movie-count", style={"marginTop": "20px", "fontSize": "16px", "color": "#444", "textAlign": "center"}),
        dcc.Graph(id="main-graph")
    ])

    children = [header, html.Hr(), cat_label, filter_controls, filter_summary, stores, axis_dd, graph_display]
...
def generate_filter_widgets(selected, user_filters):
    ...
    for col in selected:
        ...
        if pd.api.types.is_numeric_dtype(col_series):
            ...
            widgets.append(html.Div([html.Label(f"Select {col.replace('_', ' ').title()}"), dropdown]))
    return widgets
...
def update_movie_count(data):
    return f"{len(data)} movies match your filters." if data else "No movies match your filters."
...
def update_axis_options(data):
    ...
    possible = ["rating", "worldwide_gross", "production_budget", "decade", "certificate", "genre_name"]
...
def update_main_graph(_, data, x, y):
    if not data or not x or not y:
        return px.scatter(title="Select X and Y axes to visualize movie data.")
    ...
    if pd.api.types.is_numeric_dtype(x_type) and pd.api.types.is_numeric_dtype(y_type):
        ...
        fig.update_layout(title=f"{x.replace('_',' ').title()} vs {y.replace('_',' ').title()}")
    elif not pd.api.types.is_numeric_dtype(x_type) and not pd.api.types.is_numeric_dtype(y_type):
        if x == "decade":
        ...
        fig = px.bar(grouped, x=x, y="count", color=y, barmode="stack",
                     labels={"count": "Count", x: x.replace("_", " ").title(), y: y.replace("_", " ").title()},
        ...
        fig = px.bar(grouped, x=x, y=y,
                     labels={x: x.replace("_", " ").title(), y: f"Avg {y.replace('_',' ').title()}"},
